...
async function main() {
  ...
  const users = await Promise.all([
    ...
    prisma.user.create({ data: { username: "resell_king", email: "resell_king@example.com", passwordHash: hash } }),
    prisma.user.create({ data: { username: "collector_jay", email: "collector_jay@example.com", passwordHash: hash } }),
  ]);
